...
interface TerminalSplitCompositorOptions {
   ...
   /** Shared scroll state for the bash transcript. */
   transcriptScrollState?: { offset: number; maxScroll: number };
}
...
export function buildFixedClusterPaint(
   ...
   const bottomStartRow = Math.max(1, terminalRows - nonTranscriptCount + 1);
   ...
   if (cluster.cursor && showHardwareCursor) {
      ...
      if (cursorRow >= tStart && cursorRow < tEnd) {
         ...
         const adjustedRow = cursorRow >= tEnd ? cursorRow - transcriptCount : cursorRow;
         buffer += moveCursor(bottomStartRow + adjustedRow, Math.max(1, cluster.cursor.col + 1));
      }
      buffer += showCursor();
   } else {
      buffer += hideCursor();
...
export class TerminalSplitCompositor {
   ...
   private readonly transcriptScrollState: { offset: number; maxScroll: number } | null;
   private transcriptStartRow = -1;
   ...
   constructor(options: TerminalSplitCompositorOptions) {
      ...
      this.transcriptScrollState = options.transcriptScrollState ?? null;
   ...
   requestRepaint(): void {
      ...
      const prevClusterLineCount = this.visibleClusterLines.length;
      const prevTranscriptRowCount = this.transcriptRowCount;
      ...
      // If the cluster shrank, clear the newly exposed rows.
      if (newScrollableRows > prevScrollableRows) {
         for (let i = prevScrollableRows; i < newScrollableRows; i++) {
            buffer += moveCursor(i + 1, 1);
            buffer += clearLine();
         }
      }
   ...
   private handleInput(data: string): { consume?: boolean; data?: string } | undefined {
      ...
      if (
         ...
         (matchesKey(data, "ctrl+shift+c") || matchesKey(data, "ctrl+c"))
      ) {
         this.onCopySelection?.(this.getSelectedText());
         return { consume: true };
      }

      if (!isKeyRelease(data) && matchesKey(data, "end") && this.scrollOffset > 0) {
   ...
   private handleMousePacket(packet: SgrMousePacket): void {
      ...
      if (delta !== 0) {
         ...
         if (this.isMouseOverTranscript(packet.row)) {
   ...
   private selectionLocationForPacket(packet: SgrMousePacket): SelectionLocation | null {
      ...
      if (packet.row <= this.visibleScrollableRows) {
         // If the transcript overlay is active, check whether the click lands
         // on actual transcript content (at the bottom of the chat area) or
         // blank space above it.
         if (this.transcriptStartRow >= 0 && this.transcriptRowCount > 0) {
            const overlayStart = this.visibleScrollableRows - this.transcriptRowCount + 1;
            if (packet.row >= overlayStart && packet.row <= this.visibleScrollableRows) {
               return {
                  area: "cluster",
                  point: { line: this.transcriptStartRow + packet.row - overlayStart, col },
               };
            }
            // Over blank overlay area — no selection.
            return null;
         }
         return {
            area: "root",
      ...
      // Non-transcript portion starts after transcript rows in the cluster.
      const adjustedLine = clusterLine + this.transcriptRowCount;
      if (adjustedLine >= this.visibleClusterLines.length) return null;

      return {
         area: "cluster",
         point: { line: adjustedLine, col },
      };
   }
   ...
   private clampedSelectionPointForPacket(packet: SgrMousePacket, area: SelectionArea | null): SelectionPoint {
      if (area === "cluster") {
         ...
         const adjustedLine = clusterLine + this.transcriptRowCount;
         return {
            line: Math.max(0, Math.min(adjustedLine, this.visibleClusterLines.length - 1)),
            col: Math.max(0, packet.col - 1),
         };
      }
   ...
   private isMouseOverTranscript(mouseRow: number): boolean {
      // The transcript overlay covers the entire scrollable area when active.
      if (!this.transcriptScrollState || this.transcriptStartRow < 0) return false;
      return mouseRow >= 1 && mouseRow <= this.visibleScrollableRows;
   }

   private scrollTranscript(delta: number): void {
      if (!this.transcriptScrollState) return;
      const nextOffset = Math.max(
         0,
         Math.min(this.transcriptScrollState.offset + delta, this.transcriptScrollState.maxScroll),
      );
      if (nextOffset === this.transcriptScrollState.offset) return;
      this.transcriptScrollState.offset = nextOffset;
      this.requestRepaint();
   }
   ...
   private getCluster(width: number, terminalRows: number): FixedEditorClusterRender {
      if (
         ...
         this.transcriptStartRow = this.renderPassCluster.cluster.transcriptStartRow ?? -1;
         this.transcriptRowCount = this.renderPassCluster.cluster.transcriptRowCount ?? 0;
      ...
      this.transcriptStartRow = cluster.transcriptStartRow ?? -1;
      this.transcriptRowCount = cluster.transcriptRowCount ?? 0;
