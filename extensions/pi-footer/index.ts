...
import { parsePowerlineConfig } from "./powerline-config";
import { invalidateGitStatus, invalidateGitBranch } from "./git-status";
...
import { followSubmittedEditorToBottom } from "./chat-jump";
import { type RenderDeps, type LayoutCache, createRenderFunctions } from "./powerline-render";
import {
...
export default function powerlineFooter(pi: ExtensionAPI) {
   ...
   const renderDeps: RenderDeps = {
      ...
      get transcriptScrollState() {
         return fixedEditorState.transcriptScrollState;
      },
      get showLastPrompt() {
   ...
   const getShellPath = () => {
      ...
      try {
         ...
         return process.platform === "win32" ? "bash" : "/bin/sh";
      }
   };
   ...
   const fixedEditorContext: FixedEditorContext = {
      ...
      getBashModeActive: () => bashModeActive,
      requestStatusRender,
      resetLayoutCache,
   };
   ...
   const setBashModeActive = async (value: boolean, ctx: any): Promise<void> => {
      ...
      fixedEditorState.transcriptScrollState.offset = 0;
      fixedEditorState.transcriptScrollState.totalLines = 0;
      fixedEditorState.transcriptScrollState.prevCommandCount = 0;
      currentEditor?.dismissBashModeUi?.();
      requestStatusRender();
   ...
   pi.registerCommand("powerline", {
      description: "Enable or disable powerline (on/off)",
      handler: async (args, ctx) => {
         ...
         ctx.ui.notify("Usage: /powerline [on|off]", "info");
      },
   });
