...
const modelSegment: StatusLineSegment = {
   ...
   render(ctx) {
      ...
      const modelId = ctx.model?.id || ctx.model?.name || "no-model";
      const providerName = ctx.model?.provider?.trim();
      const label = providerName ? `${providerName}/${modelId}` : modelId;
...
const contextPctSegment: StatusLineSegment = {
   ...
   render(ctx) {
      ...
      const text = `${formatTokens(used)}/${formatTokens(window)} (${pct.toFixed(1)}%)${autoIcon}`;

      return { content: ctx.theme.fg("dim", text), visible: true };
   },
};
...
const skillsSegment: StatusLineSegment = {
   id: "skills",
   render(ctx) {
      if (!ctx.skillsTotal) return { content: "", visible: false };
      return {
         content: ctx.theme.fg("dim", `skills ${ctx.skillsActive}/${ctx.skillsTotal}`),
         visible: true,
      };
   },
};
...
const mcpSegment: StatusLineSegment = {
   id: "mcp",
   render(ctx) {
      const raw = ctx.extensionStatuses.get("mcp");
      if (!raw) return { content: "", visible: false };
      const normalized = normalizeExtensionStatusValue(raw);
      if (!normalized) return { content: "", visible: false };

      const match = normalized.match(/MCP:\s*(\d+)\/(\d+)/i);
      const content = match ? `${match[1]}/${match[2]} mcps` : normalized.replace(/^MCP:\s*/i, "");
      return { content: ctx.theme.fg("dim", content), visible: true };
   },
};
